        ...
        {showPlagiarism && (
          <div className="w-80 border-l border-border bg-card flex flex-col animate-slide-in-right overflow-auto">
            ...
            <div className="p-4 space-y-4">
              <Button className="w-full" size="sm" onClick={runPlagiarismCheck} disabled={plagiarismRunning} variant={plagiarismRunning ? 'secondary' : 'default'}>
                {plagiarismRunning ? (
              ...
              {plagiarismReport && (
                <>
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed">{plagiarismReport.summary}</p>
                  <button
                  ...
                  {plagiarismHighlightsVisible && plagiarismReport.flagged_passages.length > 0 && (
                    <div className="space-y-3">
                      <h4 className="text-xs font-medium text-foreground">Flagged Passages ({plagiarismReport.flagged_passages.length})</h4>
                      {plagiarismReport.flagged_passages.map((passage, i) => (
                        <div key={i} className={`rounded-lg border p-3 space-y-2 ${getSeverityColor(passage.severity)}`}>
                          <div className="flex items-center justify-between">
                            <span className="text-[10px] uppercase tracking-wider font-medium text-muted-foreground">{passage.concern_type.replace('_', ' ')}</span>
                            ...
                            }`}>{passage.severity}</span>
                          </div>
                          <p className="text-xs text-foreground/80 italic leading-relaxed">"{passage.excerpt}"</p>
                          <p className="text-[11px] text-muted-foreground leading-relaxed">{passage.reason}</p>
                        </div>
                      ))}
                    </div>
                  )}
